...
const NavBar = ({ selectedStep, setSelectedStep, awakened}: StepperProps) => {
    ...
    const getStepper = () => {
        return (
            <Stepper orientation="vertical" color="blue" size="xs" active={selectedStep} onStepClick={setSelectedStep} breakpoint="sm">
                <Stepper.Step key={"Basics"} label={"Intro"} description=""> </Stepper.Step>
                <Stepper.Step key={"Attributes"} label={"Attributes"} description="" disabled={getDisabledConditions(1)}> </Stepper.Step>
                <Stepper.Step key={"Skills"} label={"Skills"} description="" disabled={getDisabledConditions(2)}> </Stepper.Step>
                <Stepper.Step key={"Path"} label={"Path"} description="" disabled={getDisabledConditions(3)}> </Stepper.Step>
                <Stepper.Step key={"Order"} label={"Order"} description="" disabled={getDisabledConditions(4)}> </Stepper.Step>
                <Stepper.Step key={"Arcana"} label={"Arcana"} description="" disabled={getDisabledConditions(5)}> </Stepper.Step>
                <Stepper.Step key={"Merits"} label={"Merits"} description="" disabled={getDisabledConditions(6)}> </Stepper.Step>
                <Stepper.Step key={"Experience"} label={"Experience"} description="" disabled={getDisabledConditions(7)}> </Stepper.Step>
                <Stepper.Step key={"Finishing Touches"} label={"Finishing Touches"} description="" disabled={getDisabledConditions(8)}> </Stepper.Step>
                <Stepper.Step key={"Confirm"} label={"Confirm"} description="" disabled={getDisabledConditions(8)}></Stepper.Step>
            </Stepper>
        )
    }
